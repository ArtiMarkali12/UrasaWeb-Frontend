...
// ── HERO CAROUSEL IMAGES (3 images) ───────────────────────────
import heroImage1 from "../../assets/images/HomeImages/hero.jpeg";
import heroImage2 from "../../assets/images/HomeImages/hero.jpeg";
import heroImage3 from "../../assets/images/HomeImages/hero.jpeg";
...
───────────────────────────────────────────────────────────── */
const HERO_SLIDES = [
  { id: 1, image: heroImage1, alt: "Urasa Arts printing facility — slide 1" },
  { id: 2, image: heroImage2, alt: "Urasa Arts printing facility — slide 2" },
  { id: 3, image: heroImage3, alt: "Urasa Arts printing facility — slide 3" },
];
...
/* ─────────────────────────────────────────────────────────────
   TOUCH DELAY CONFIG
   ─────────────────────────────────────────────────────────────
   प्रत्येक card type साठी वेगळा delay (ms):
   - triple (envelope) : 2800ms — animation 1.6s + spread state hold
   - six               : 2200ms — explode animation पूर्ण दिसेल
   - dual / dual2/dual4: 1800ms — slide apart animation दिसेल
   - single            : 1400ms — float up animation दिसेल
───────────────────────────────────────────────────────────── */
const TOUCH_DELAYS = {
  triple: 2800,
  six:    2200,
    ...
    >
      ...
      <div className="hero-dots" role="tablist" aria-label="Slide indicators">
        {HERO_SLIDES.map((slide, idx) => (
          ...
          >
            {idx === current && (
              <span className="hero-dot-fill" style={{ width: `${progress}%` }} />
            )}
          </button>
        ))}
      </div>
...
const Home = () => {
  ...
  // ── per-card touch timer refs (prevent memory leaks) ──────
  ...
  /* ─────────────────────────────────────────────────────────
     handleTouchStart
     - Clears any existing timer for this card (re-tap resets)
     - Adds svc-touched immediately
  ───────────────────────────────────────────────────────── */
  const handleTouchStart = useCallback((e, cardId) => {
    const card = e.currentTarget;
    if (touchTimers.current[cardId]) {
      clearTimeout(touchTimers.current[cardId]);
      delete touchTimers.current[cardId];
    }
    card.classList.add("svc-touched");
  }, []);

  /* ─────────────────────────────────────────────────────────
     handleTouchEnd
     - Waits for card-type-specific delay then removes class
     - TOUCH_DELAYS config at top of file controls timing
  ───────────────────────────────────────────────────────── */
  const handleTouchEnd = useCallback((e, cardId, cardType) => {
    const card  = e.currentTarget;
    const delay = TOUCH_DELAYS[cardType] ?? 1800;

    if (touchTimers.current[cardId]) {
      clearTimeout(touchTimers.current[cardId]);
    }

    touchTimers.current[cardId] = setTimeout(() => {
      card.classList.remove("svc-touched");
      delete touchTimers.current[cardId];
    }, delay);
  }, []);
          ...
          <div className="svc-grid" ref={gridRef}>
            {services.map((svc, index) => (
              <div
                ...
                className={`svc-card svc-card--${svc.type}${cardsVisible ? " svc-visible" : ""}`}
                style={{ transitionDelay: cardsVisible ? `${index * 0.1}s` : "0s" }}
                onTouchStart={(e) => handleTouchStart(e, svc.id)}
                onTouchEnd={(e)   => handleTouchEnd(e, svc.id, svc.type)}
              >
                <div className="svc-img-wrap">
                  {svc.images.map((img) => (
                    <img
      ...
      <section className="recent-section">
        <div className="recent-container">
          <div className="recent-header">
            <h2>Our Recent Work</h2>
            <p>
              Discover our latest print and manufacturing achievements. We
              continuously update this gallery with our newest projects—from
              complex publishing to premium packaging. Explore our most recent
              collaborations to stay informed on our evolving capabilities and
              our ongoing commitment to uncompromising precision.
            </p>
          </div>
          <div className="recent-grid">
            {[
        ...
        <div className="cs-inner">
          ...
          >
            ...
            >
              <div className="cs-rows" style={{ width: "100%" }}>
                {CLIENT_ROWS.map((row) => (
                  <ClientMarqueeRow key={row.id} id={row.id} logos={row.logos} />
                ))}
              </div>
            </div>
          </div>
